...
def test_create_post(client):
    ...
    payload = {
        ...
        'description': '{} {}'.format(get_random_sentence(), get_random_sentence()),
...
def test_get_my_posts(client):
    ...
    response = client.get(f"{settings.API_V1_STR}/posts/{user.id}/posts", headers=header)
    data = response.json()
